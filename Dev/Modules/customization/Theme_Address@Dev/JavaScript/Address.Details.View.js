	...
	)
{
	...
	,	getContext: function ()
		{
		//JJ--parentviewclassname gets the classname of the parent, whether it is shipping address module or billing address module
		var parentviewclassname= this.parentView.el.className;
		if(parentviewclassname === 'OrderWizard.Module.Address.Shipping module-rendered')
			{ 
				var ischangeaddrshipping = true; 
			}
		else{
				var ischangeaddrshipping = false;
			}
		if(parentviewclassname === 'OrderWizard.Module.Address.Billing module-rendered')
			{ 
				var ischangeaddrbilling = true;
			}
		else{
				var ischangeaddrbilling = false;
			}
			...
			,	isInvalidAddressToRemove: !!(this.options.disableRemoveButton && this.options.disableRemoveButton(this.model.get('internalid')))
				// JJ-- isselectMessage contains the value selectMessage. selectMessage has value only for shipping address.
				//,isselectMessage: isselectMessage
			,	ischangeaddrshipping:ischangeaddrshipping
			,	ischangeaddrbilling:ischangeaddrbilling
			};
		}
	});
});
