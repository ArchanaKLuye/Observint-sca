	...
	)
{
	...
	,	events: {
		...
		//,	'change #setsameship': 'setSameShipAddress'
		}
	...
	/*,	setSameShipAddress: function(e)
		{
			var shipVal = jQuery(e.target).val();
			jQuery('#shipping option[data-id='+shipVal+']').attr("selected",true);
		}*/
	...
	,	setAddress: function (address_id, options)
		{
			//console.log("#######this.addressId",this.addressId +'--' +address_id);
	...
	,	markSameAs : function (is_checked)
		{
			...
			if(is_checked){
			jQuery('#shipping option[data-id='+this.model.get(this.sameAsManage)+']').prop("selected",true);
				//jQuery('#setsameship').val(this.model.get(this.sameAsManage));
			}/*else{
				jQuery('#setsameship').val("");
			}*/
			//this.render();
		}
	...
	,	getAddressListOptions: function ()
		{console.log("this.selectMessage -inside function",this.selectMessage);
			return {
				showSelect: true
