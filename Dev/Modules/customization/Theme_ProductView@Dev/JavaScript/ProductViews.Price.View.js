	...
	)
{
	...
	,	getContext: function ()
		...
		console.log("price_container_object",price_container_object);			
			...
			var msrp =  price_container_object.compare_price ? price_container_object.compare_price : 0;
			var salePrice =  price_container_object.sale_price;
			var yourprice =  price_container_object.price;
			var savings;
			var savingpercent;
			var issavingpercent;
			...
			{  
				if( msrp > yourprice)
				{
					 savings= (msrp - salePrice).toFixed(2);
					 savingpercent= ((savings/salePrice)*100).toFixed(2);
				}
				else
				{
					 savings= (yourprice - salePrice).toFixed(2);
					 savingpercent= ((savings/salePrice)*100).toFixed(2);
				}
			}
			else if(msrp > yourprice)
			{
				 savings= (msrp - yourprice).toFixed(2);
				 savingpercent= ((savings/yourprice)*100).toFixed(2);
			}
			if(savingpercent== 0)
			{
				 issavingpercent = false;
			}
			else
			{
				 issavingpercent = true;
			}
			if(salePrice)
			{
			...
			return {
				...
				,	salePrice:price_container_object.sale_price ? price_container_object.sale_price :0

				,	salePriceFormatted:price_container_object.sale_price_formatted || ''
				...
				,	issavingpercent: issavingpercent


				//,	showYourPrice:showYourPrice
