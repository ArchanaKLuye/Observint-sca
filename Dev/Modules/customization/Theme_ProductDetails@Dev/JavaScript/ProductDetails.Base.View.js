	...
	)
{
	...
	var ProductDetailsBaseView = Backbone.View.extend(
		...
		,	childViews: {
			...
			,'ProductReviews.Center': function (options)
					{
			...
			,	getContext: function() {
                    ...
                    var getkeyfeatures = this.model.get('item').get('custitem415');
                    var keyfeatures = $("<p/>").html(getkeyfeatures).text();
                    var getvideos = (this.model.get('item').get('custitem_videos') || '0').trim().split(',');
                    var len = getvideos.length;
                    for (var i = 0; i < len; i++) {
                        var media_url = $("<p/>").html(getvideos[i]).text();
                        if (media_url.indexOf('youtube.com/') !== -1) {
                            params = Utils.parseUrlOptions(media_url);
                    ...
                    // var videos = $("<p/>").html(getvideos).text();


                    /*console.log("pdpname",this.model.get('item').get('storedisplayname2'|| 'displayname' || item_id));*/
                    ...
                    return {
                        ...
                        isPriceEnabled: !ProfileModel.getInstance().hidePrices()
                            //,	keyfeatures: this.model.get('item').get('custitem415')
                            ,
                        ...
                        itemtopname: this.model.get('item').get('custitem_itemtopname')

                            ,
                        sku: this.model.get('item').get('itemid'),
                        name: this.model.get('item').get('storedisplayname2' || 'displayname' || item_id)
