	...
	)
{
	...
	,	events: {
		...
		,	'click [data-action="delete-list-custom"]': 'DeleteList'
		,	'click [data-action="back"]': 'back'
		,	'click [data-action="selectAll-custom"]': 'selectAll'
		,	'click [data-action="unselectAll-custom"]': 'unselectAll'
		,	'click [data-action="add-item"]': 'additem'
		,	'click [data-action="append-row"]': 'appendrow'
		}
	...
	,	appendrow: function() 
	{console.log("hiiiiii");
		var row ="<div class=\"product-list-item-search-row\">";
				 row+="<div class=\"sku-id\">";
					/*row+="<label>SKU ID</label>";*/
					row+="<input class=\"sku\" type=\"text\" name=\"sku\">";
				row+="</div>";
				row+="<div class=\"qty\">";
					/*row+="<label>QTY</label>";*/
					row+="<input class=\"quantity\" type=\"Number\" name=\"quantity\">";
				row+="</div>";
			row+="</div>";
			console.log("row",row);
		jQuery("#append-row").append(row);
	}
	,	additem: function()
		{
			var itemsku = [];
			...
			$( ".sku" ).each(function( index ) {
			   itemsku[index] = $( this ).val(); 
			});
			$( ".quantity" ).each(function( index ) {
			   quantity[index] = $( this ).val(); 
			});

			/*var itemsku = jQuery(".sku").val();
			console.log("itemsku",itemsku);
			var quantity = jQuery(".quantity").val();
			console.log("quantity",quantity);*/
			var id = this.model.attributes.internalid;
			this.itemstolistmodel  = new AddItemsToListModel();
			this.itemstolistmodel.fetch({
				data:{
					id : id,
	...
	,	getContext: function()
		{
			...
			,	lastmodifieddate: this.model.attributes.lastmodifieddate
			};
		}
	});
});
