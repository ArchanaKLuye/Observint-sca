...
<a class="header-profile-welcome-link" href="#" data-toggle="dropdown">
	...
	</a> {{#if showMyAccountMenu}}
<div class="header-profile-menu-myaccount-container header-profile">
    <div data-view="Header.Menu.MyAccount.Home"></div>
</div>
{{/if}} {{else}} {{#if showLoginMenu}} {{#if showLogin}}
...
<a class="header-profile-loading-link">
		...
		</a> {{/if}} {{/if}} {{!---- Use the following context variables when customizing this template: showExtendedMenu (Boolean) showLoginMenu (Boolean) showLoadingMenu (Boolean) showMyAccountMenu (Boolean) displayName (String) showLogin (Boolean) showRegister (Boolean) ----}}