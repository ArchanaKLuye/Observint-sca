<button class="accordion-section-payment-method"  onclick="myFunction4()">
	 <span class="serial-payment-method">4</span>
		<h2 class="order-wizard-paymentmethod-selector-module-header" >
			PAYMENT METHOD<!-- {{title}} -->
		</h2>
		<!-- <span class="fa fa-minus payment-method-check-plus" id="accor-plus-payment-method"></span> -->
		<span class="minu-accc-section-payment-method" id="minu-accc-section-payment-method"><img class="payment-method-accordion" src="/SSP Applications/NetSuite Inc. - SCA Aconcagua/Development/img/minus.png"></span>
</button>
...
<div class="order-wizard-paymentmethod-selector-module" >
	...
	<!-- <span class="creditcard-edit-form-label-required">*</span><div class='order-wizard-paymentmethod-selector-module-options-label'>Credit Card Type</div> -->


		<!-- <select class="order-wizard-paymentmethod-selector-module-options" id="credit-card-type"data-action="select-payment-method" >
			{{#each activeModules}}
		...
		<div data-action="select-payment-method" class="paymentmethod-selection-area">
			{{#each activeModules}}
			<div class="col-sm-6 col-md-6 col-lg-6 paymentmethods">
			<input type="radio" {{#if isSelected}}checked{{/if}}  name="paymentmethod" value="{{type}}">{{name}}</div>
			{{/each}}
		</div>
...
<script type="text/javascript">
  ...
  var x = document.getElementById("info-section-payment-method");
  $(".payment-method-accordion").hide();
  if (x.style.display === "none") {
    x.style.display = "block";
    //$('.order-wizard-paymentmethod-selector-module').css('display','none');
    $('.order-wizard-paymentmethod-purchasenumber-module').css('display','block');
     $("#minu-accc-section-payment-method").removeClass("plus").addClass("minus");
  } else {
    x.style.display = "none";
    //$('.order-wizard-paymentmethod-selector-module').css('display','none');
     $('.order-wizard-paymentmethod-purchasenumber-module').css('display','none');
    $("#minu-accc-section-payment-method").removeClass("minus").addClass("plus");
  }
}
